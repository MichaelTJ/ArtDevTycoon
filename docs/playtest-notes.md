...
| P5  | Bug     | After painting, focusing “your prompt” still lets Phaser eat keys — typing/WASD moves the character instead of inserting letters                                                                                              | **17** Phaser input capture + **04** integration (prompt field focus)                               | **21f** QoL if we treat it as floor input polish                          |
...
| P10 | Bug     | ~~**Crayon Mode** notice stays up after selecting **Janus**~~ **Fixed** — notice now gates on active mock engine, not device capability                                                                                       | **04** Integration — `CapabilityNotice` / `EngineStore` (`showCrayonNotice`)                        | Secondary: **05** / **07** if availability lags after `select()`          |

## Suggested fix order (when we start)

1. **P5** — input focus (blocks typing/play)
