...
import { SdTurboWorkerClient, type WorkerLike } from './workerClient';
...
describe('SdTurboWorkerClient', () => {
	it('resolves load when loaded arrives', async () => {
		const fake = new FakeWorker();
		fake.register('load', (request) => [{ type: 'loaded', id: request.id }]);
		const client = new SdTurboWorkerClient(() => fake as unknown as Worker);

		await expect(client.load()).resolves.toBeUndefined();
		expect(fake.posted[0]).toMatchObject({ type: 'load', engineId: 'sdturbo-webgpu' });
	});
	...
	it('resolves disposeSessions when unloaded arrives', async () => {
		const fake = new FakeWorker();
		fake.register('unload', (request) => [{ type: 'unloaded', id: request.id }]);
		const client = new SdTurboWorkerClient(() => fake as unknown as Worker);

		await expect(client.disposeSessions()).resolves.toBeUndefined();
		expect(fake.posted[0]).toMatchObject({ type: 'unload' });
	});
	...
	it('does not interleave concurrent generate calls', async () => {
		...
		const client = new SdTurboWorkerClient(() => fake as unknown as Worker);
		const first = client.generate('prompt-a', 1);
		const second = client.generate('prompt-b', 2);
	...
	it('rejects with cancelled when abort signal fires', async () => {
		...
		await expect(client.generate('prompt', 1, undefined, controller.signal)).rejects.toMatchObject({
			code: 'cancelled'
		});
	});
});
