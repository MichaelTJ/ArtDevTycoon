...
/**
 * Promise-based RPC wrapper over the SD-Turbo inference worker.
 * Serialises requests so two generations never overlap in VRAM.
 */
export class SdTurboWorkerClient {
	...
	constructor(factory?: () => Worker) {
		this.worker =
			factory?.() ??
			new Worker(new URL('./sdturbo.worker.ts', import.meta.url), { type: 'module' });
		this.worker.addEventListener('message', this.handleMessage);
	}

	/** Download and compile the three ORT sessions in the worker. Safe to call twice. */
	load(
	...
	/**
	 * Generate a 512×512 image from the built prompt only — the player's raw text never
	...
	/**
	 * Release ORT sessions without terminating the worker so Janus can load for critique.
	 * Sessions are recreated lazily on the next `load()` or `generate()` call.
	 */
	disposeSessions(signal?: AbortSignal): Promise<void> {
		return this.enqueue<void>('dispose', { signal });
	}
	...
	private dispatchRequest<T>(
		...
		return new Promise<T>((resolve, reject) => {
			...
			if (kind === 'dispose') {
				this.worker.postMessage({ type: 'unload', id });
				return;
			}
