...
| `GeneratingPanel`      | `messages?`, `intervalMs?`, `progress?`, `stageLabel?`, `compact?` — compact is a narrow sidebar (spinner or % + rotating line; no image placeholder)                                                                                                                                                                                                                                | —                                                                                                                                                                                                            |
...
| `GenerationReadyToast` | Spec 31 — `onopen`. Compact **Finished!** / **Click here to compare drawings.** card beside the commission canvas.                                                                                                                                                                                                                                                                   | `onopen()`                                                                                                                                                                                                   |
| `CritiqueSentToast`    | `clientName` — fixed float after the player submits AI or drawing. Copy: **Sent for critique** / Sending your painting to {name} for a critique.                                                                                                                                                                                                                                     | —                                                                                                                                                                                                            |
...
`SketchCanvas` is a presentational 384×384 paint surface. **Spec 31:** tools and colours sit in a left column; the canvas is centered on the right. During **generating**, a compact `GeneratingPanel` sits to the right of the canvas (spinner or % — no grey image placeholder). When generate finishes that column becomes **Finished!** (opens compare) so the canvas does not jump. A compact medium XP bar in the footer ticks from brush movement the same way as Practice. After generate (`pendingSubmitChoice`), the canvas **stays mounted**. Parent registers the PNG exporter via `onsketchexportready`. Better mediums get more colours except Ink & Charcoal (B&W only): crayon 8 swatches, pencil 14, then watercolor/acrylic/oil RGB wells (Spec 32). No Custom colour input. Oil on Canvas shows **Oil brush** kinds (Round / Bristle / Flat / Palette knife). Undo stack depth is 20; clear wipes to white and clears `hasStrokes`.
