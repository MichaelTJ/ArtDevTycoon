...
| `MyPcSetup`            | `provider`/`baseUrl`/`apiKey`/`generateModel`/`critiqueModel`/`critiqueProvider`/`critiqueBaseUrl` (bindable), `availableModels`, `testState`, `testError` | `onproviderchange?`, `onrefreshmodels()`, `ontest()`, `onconnect()`, `oncancel()`             |
