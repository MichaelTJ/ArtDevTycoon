...
**Spec 25:** `mediumTierId` selects a `brushProfiles` stroke feel (crayon grain, pencil thin lines, ink charcoal grain + bleed + B&W palette, watercolour wash). `StudioHudOverlay` shows a **Painting medium** picker during `generating`; selection syncs via `onselectmedium` → `game.setActiveMediumTier`. Locked tiers are disabled with cash/rep tease.
