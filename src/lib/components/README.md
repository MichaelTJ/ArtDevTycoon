...
| `StudioHudOverlay`     | phase, client/artwork/critique, `activeMediumTierId?`, `unlockedMediumTierIds?`, `cash?`, `reputation?`, `mumRealCritique?`, `draftPrompt` (bindable), `pendingSubmitChoice?`, `aiGeneratedImageUrl?`, `clientSummoned`, `floorInteract?`, `pendingSkillGains?`, `practiceOpen?`, `skill?`, `onsketchexportready?`, `onconfirmsubmit?`, `studioDebug?` | `oninvite` / `ontalk` / `ondeliver` / `onsubmit` / `oncollect` / `onretry` / `ondismisserror` / `ondecline?` / `onselectmedium?` / `onpractice?` / `onpracticetick?` / `onexitpractice?` |
