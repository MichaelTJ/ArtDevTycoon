...
| `generating`    | Viewport dialog: {@link SketchCanvas} (paint while waiting; stays mounted after AI arrives) + {@link GenerationReadyToast} + {@link SubmitCompareModal} until `confirmSubmitChoice` + **Skip** | `createArt()` then player picks                               |
