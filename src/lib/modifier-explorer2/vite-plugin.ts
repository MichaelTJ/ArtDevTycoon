...
const OUTPUT_DIR = path.resolve('data/modifier-explorer2');
...
function registerRoutes(server: ViteDevServer): void {
	server.middlewares.use(async (req, res, next) => {
		...
		if (url.startsWith('/modifier-explorer2-output/')) {
			// Strip ?v= epoch (and any hash) so cache-bust query params don't break disk lookup.
			const pathOnly = url.split('?')[0]?.split('#')[0] ?? url;
			const relative = decodeURIComponent(
				pathOnly.slice('/modifier-explorer2-output/'.length)
			);
		...
		if (url === '/api/modifier-explorer2/manifest' && req.method === 'GET') {
			try {
				await ensureOutputDir();
		...
		if (url === '/api/modifier-explorer2/save' && req.method === 'POST') {
			try {
				await ensureOutputDir();
		...
		if (url === '/api/modifier-explorer2/good-tags' && req.method === 'POST') {
			try {
		...
		if (url === '/api/modifier-explorer2/wipe' && req.method === 'POST') {
			try {
...
/** Dev-only disk persistence for the modifier explorer 2 gallery. */
export function modifierExplorer2Plugin(): Plugin {
	return {
		name: 'modifier-explorer2',
		// PNG + manifest writes must not trigger a full-page Vite reload mid-batch.
		config: () => ({
			server: {
				watch: {
					ignored: ['**/data/modifier-explorer2/**']
				}
			}
		}),
