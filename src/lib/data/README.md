...
| `sketchPalettes.ts`    | Spec 32 — `CRAYON_PALETTE` / `INK_PALETTE` / `WATERCOLOR_PALETTE` / `ACRYLIC_PALETTE` / `OIL_PALETTE`, `RGB_PICKER_MEDIUM_IDS`, `OIL_STROKE_KINDS` / `OIL_STROKE_LABELS`, `swatchesForMedium`, `showsRgbPicker`, `showsCustomColour`, `showsOilStrokePicker`, `hexFromRgb`, `rgbFromHex` |
